...
class MandelbrotCustomCanvasChannelIncremental {

    companion object {
        private const val MAX_ITER = 300
        private const val ZOOM = 300
        private val CORE = 100
    }
    ...
    private fun parallel(canvas: MandelbrotCanvas, update: () -> Unit) = CoroutineScope(Dispatchers.Default).launch {
        ...
        accumulateConsumer(consumer, canvas, update).join()
    }

    private fun CoroutineScope.accumulateProducer(
    ...
    private fun CoroutineScope.accumulateConsumer(
        consumer: Channel<List<MandelbrotPoint>>,
        canvas: MandelbrotCanvas,
        update: () -> Unit
    ) = launch {
    ...
    fun draw(canvas: MandelbrotCanvas, update: () -> Unit = { }) {
